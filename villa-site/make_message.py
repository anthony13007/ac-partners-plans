...
And this is the kind of site we build for our villas to sell direct, scroll it on your phone: {showcase}
...
15 minutes on WhatsApp this week? {days[0]} or {days[1]}?"""
    ...
    fallback = f"""Understood, no problem. One lighter option, zero commitment: we feature {villa} as a highlight on the {s['brand']} guest site and in our guest emails (repeat guests, groups, retreats looking for large villas). You handle the booking directly, we take {round(d['highlight_commission']*100)}% on a booking we send you, nothing otherwise. Shall I add it?"""

    fu3 = f"""Hi {first}, did the page load OK on your side? {url}
Happy to walk you through the {g['empty']} nights in 10 min."""
    fu7 = f"""{first}, one number from our side: {s['proof']}. Same channels, same pricing engine, for {villa}. 10 min this week?"""
    fu14 = f"""Last one from me, {first}. We take a handful of villas per area for the low season. Want the spot for {villa}, or shall I pass? The site stays here either way: {site_url}"""

    today = datetime.date.today()
    md = f"""# Outreach — {villa} ({area}, {br}BR)

Generated {today.isoformat()} · owner page: {url} · guest site: {site_url} · showcase: {showcase}
Numbers: {g['empty']} empty nights / {g['nights']} in {months_txt} (market {occ}%), {loss} not earned at {adr}/night; we target +{g['target']} nights = {money(g['added'], cur)} added, our fee {money(g['fee'], cur)}, guarantee {guarantee} nights.
...
## Follow-up J+3 after message 2 ({(today + datetime.timedelta(days=3)).isoformat()})
```
{fu3}
```

## Follow-up J+7 ({(today + datetime.timedelta(days=7)).isoformat()}) — attach a PriceLabs screenshot of one of our villas vs market
```
{fu7}
```
